...
sMM_TQ = np.zeros((len(col),len(lon)))
sDD_TQ = np.zeros((len(col),len(lon)))
scc_TQ = np.zeros((len(col),len(lon)))
sll_TQ = np.zeros((len(col),len(lon)))
scl_TQ = np.zeros((len(col),len(lon)))
sMM_LISA = np.zeros((len(col),len(lon)))
sDD_LISA = np.zeros((len(col),len(lon)))
...
for m in M[19:]:
	...
	# Generate the on and off times for the mass case
	OO_M = Detection.OO_LISA(t)
	...
	# Generate the on and off times for the other cases
	OO_o = Detection.OO_LISA(t)
	...
	for i in range(len(col)):
		for j in range(len(lon)):
			...
			# For the mass and the luminosity distance
			f, dh_M = Detection.fourier_LISA(t, dhp_M, dhc_M, col[i], lon[j], f_min, f_max, OO_M)
			f, dh_D = Detection.fourier_LISA(t, dhp_D, dhc_D, col[i], lon[j], f_min, f_max, OO_o)


			# Compute the Fourier transformed differentiation over the sky positions for TianQin
			f, h_cp = Detection.fourier_LISA(t, hp, hc, col[i]*(1+1e3*delt), lon[j], f_min, f_max, OO_o)
			f, h_cm = Detection.fourier_LISA(t, hp, hc, col[i]*(1-1e3*delt), lon[j], f_min, f_max, OO_o)
			dh_c = (h_cp - h_cm)/(2*col[i]*delt)

			f, h_lp = Detection.fourier_LISA(t, hp, hc, col[i], lon[j]*(1+delt), f_min, f_max, OO_o)
			f, h_lm = Detection.fourier_LISA(t, hp, hc, col[i], lon[j]*(1-delt), f_min, f_max, OO_o)
			dh_l = (h_lp - h_lm)/(2*lon[i]*delt)
			...
			# Compute and safe the elements of the Fisher matrix for LISA
		...
		cnt +=1
		time_now = time.time()
		print('Progress:', round(100*cnt/18,1), '%')
		print('Time passed:', round((time_now-time_start)/60,1), 'min')

	# Save the detection accuracy in dat-files (create folders 'FM_TianQin' and 'FM_LISA' to save the files)
	
	# Save sigma for the mass in TianQin
	with open('FM_TianQin/sigMM_TQ_{}.dat'.format(log10(m)), 'w', newline='') as f:
		for i in range(len(col)):
			writer = csv.writer(f, delimiter=' ')
			writer.writerow(sMM_TQ[i])

	# Save sigma for the distance in TianQin
	with open('FM_TianQin/sigDD_TQ_{}.dat'.format(log10(m)), 'w', newline='') as f:
		for i in range(len(col)):
			writer = csv.writer(f, delimiter=' ')
			writer.writerow(sDD_TQ[i])

	# Save sigma for the colatitude in TianQin
	with open('FM_TianQin/sigcc_TQ_{}.dat'.format(log10(m)), 'w', newline='') as f:
		for i in range(len(col)):
			writer = csv.writer(f, delimiter=' ')
			writer.writerow(scc_TQ[i])

	# Save sigma for the longitude in TianQin
	with open('FM_TianQin/sigll_TQ_{}.dat'.format(log10(m)), 'w', newline='') as f:
		for i in range(len(col)):
			writer = csv.writer(f, delimiter=' ')
			writer.writerow(sll_TQ[i])

	# Save diagonal element for colatitude and longitude in TianQin
	with open('FM_TianQin/sigcl_TQ_{}.dat'.format(log10(m)), 'w', newline='') as f:
		for i in range(len(col)):
			writer = csv.writer(f, delimiter=' ')
			writer.writerow(scl_TQ[i])
	

	# Save sigma for the mass in LISA
	with open('FM_LISA/sigMM_LISA_{}.dat'.format(log10(m)), 'w', newline='') as f:
		for i in range(len(col)):
			writer = csv.writer(f, delimiter=' ')
			writer.writerow(sMM_LISA[i])

	# Save sigma for the distance in LISA
	with open('FM_LISA/sigDD_LISA_{}.dat'.format(log10(m)), 'w', newline='') as f:
		for i in range(len(col)):
			writer = csv.writer(f, delimiter=' ')
			writer.writerow(sDD_LISA[i])

	# Save sigma for the colatitude in LISA
	with open('FM_LISA/sigcc_LISA_{}.dat'.format(log10(m)), 'w', newline='') as f:
		for i in range(len(col)):
			writer = csv.writer(f, delimiter=' ')
			writer.writerow(scc_LISA[i])

	# Save sigma for the longitude in LISA
	with open('FM_LISA/sigll_LISA_{}.dat'.format(log10(m)), 'w', newline='') as f:
		for i in range(len(col)):
			writer = csv.writer(f, delimiter=' ')
			writer.writerow(sll_LISA[i])

	# Save diagonal element for colatitude and longitude in LISA
	with open('FM_LISA/sigcl_LISA_{}.dat'.format(log10(m)), 'w', newline='') as f:
		for i in range(len(col)):
			writer = csv.writer(f, delimiter=' ')
